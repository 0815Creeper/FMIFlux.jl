...
# ╔═╡ 7b000e76-3e71-408c-b1e3-cdbfb7b0e0e8
#fix output above cell according to https://github.com/fonsp/Pluto.jl/issues/205#issuecomment-1915737027
using HypertextLiteral 

# ╔═╡ 95ae0628-319a-494d-bec8-6f018cabec30
# Loading the required libraries
using FMI           # import FMUs into Julia

# ╔═╡ 03e0fd1b-ce0e-457c-8ddb-fd40c7c6d662
using FMIFlux       # for NeuralFMUs

# ╔═╡ 0849ca61-c8da-4661-90bd-33c21f6850f5
using FMIZoo        # a collection of demo models, including the VLDM

# ╔═╡ b52ecf87-284a-470d-984a-52f63dd464ff
using FMIFlux.Flux  # Machine Learning in Julia

# ╔═╡ c8c37289-10f7-44c6-99c3-fa27e4f052b8
using Plots         # plotting results

# ╔═╡ 11c0f8b0-12ec-460f-8d9b-cb383c96196b
using LaTeXStrings  # needed for pretty printing in helper file

# ╔═╡ ab39f4f1-7304-4069-be19-3c439ba75e80
# a helper file with some predefined functions to make "things look nicer", but are not really relevant to the topic
include(joinpath(@__DIR__, "plutocon_2023_helpers.jl"));

# ╔═╡ 1a27adad-b337-4aa2-900d-abf084e4a576
@htl """
<style>
pluto-cell {
	display: flex;
...
# ╔═╡ 44eb628c-6eea-4d8e-9f70-b5ba74338e26
md"""
...
# ╔═╡ 7691e5dc-d4cf-46d2-b004-f57a3e228aa5
...
# ╔═╡ 13356e4e-6861-41d0-a10c-874d552c0f5a
md"""
## Introduction
...
2. As a module (see sections *Optional: Organize as module*) together with the file `plutocon_2023_distributedhyperopt.jl` to perform your own distributed hyperparameter optimization.
...
# ╔═╡ 57b34d16-9af3-433c-a7d3-3fbb64511062
md"""
## 1. Installing / Loading the Packages
...
# ╔═╡ 82e8b618-7635-4662-b47f-8c87ce82b623
# disable progress bars in jupyter notebook
showProgress=false;

# ╔═╡ 8285ea21-a616-4b03-a695-e5e097ca8610
md"""
## 2. Loading FMU & Data
...
# ╔═╡ db0e9923-8f40-4f13-92b4-eb4551cc8d5b
md"""
...
# ╔═╡ ddf08396-d16a-4cc3-8a38-706d3b03647d
begin
...
# ╔═╡ 9a0a425f-3c01-4642-995a-5e5da3684701
md"""
...
# ╔═╡ baca0668-c2b8-4dcb-9af9-a8d41d3203b4
...
# ╔═╡ 2d01d0de-cadf-49e4-92b4-55dab894fd2a
md"""
...
# ╔═╡ 1f36e98b-6aee-4a4b-a140-6fb8719a4c5a
...
# ╔═╡ 20bd1224-9ab7-4e57-ba47-ed6b2a7f29d5
display(resultFMU1)

# ╔═╡ 9e64ccfc-2f35-420e-b087-7af51eccd680
md"""
...
# ╔═╡ 649dd2f7-01d8-462c-a296-c84354ee12f8
begin
	# Plot the simulation results
	fig1 = plot(resultFMU1;                               # the simulation result from above 
	...
	plot!(fig1, data.cumconsumption_t, data.cumconsumption_val; label="Data", ribbon=data.cumconsumption_dev, fillalpha=0.3)
end

# ╔═╡ 54418afd-8fab-49a7-bf92-31cc5aa18785
md"""
The simulation result we already know from the introduction!
"""

# ╔═╡ 9974ed2b-826d-478b-9407-3dd3add6208f
md"""
## 3. NeuralFMU setup

![NeuralFMU](https://github.com/thummeto/FMIFlux.jl/blob/main/docs/src/examples/img/plutocon_2023/neuralfmu_topology.png?raw=true)

Equipped with data and a simulation model, we can setup the NeuralFMU as introduced in the workshop.
"""

# ╔═╡ 87cb6f4a-4a68-453e-8c2c-9b31bd91e242
md"""### 3.1 Pre- and Post-Processing
...
# ╔═╡ d1145c30-9548-4f72-a845-7e442442e530
...
# ╔═╡ 5f31a618-df26-4570-9965-8501ea13e09f
manipulatedDerVals = fmiGetSolutionValue(resultFMU1, manipulatedDerVars)

# ╔═╡ fb5a9a40-efa8-4591-9ad6-8b346dc7ccd3
# what happens without propper transformation between FMU- and ANN-domain?
plot(resultFMU1.values.t, manipulatedDerVals[1,:][1]; label="original", xlabel="t [s]", ylabel="velocity [m/s]")

# ╔═╡ a11a1b9d-1ae3-4383-85cd-83873477e6ec
md"""
But what happens if we put the velocity into the hyperbolic tangens function?
"""

# ╔═╡ 772cfd86-4ef0-48ee-a3fc-03e6bb8e413b
plot!(resultFMU1.values.t, tanh.(manipulatedDerVals[1,:][1]); label="tanh(velocity)")

# ╔═╡ f33b95c3-ab52-41a9-a574-bbf0098c8468
md"""
...
# ╔═╡ d082788c-5920-418b-b295-f048d2b4411c
...
# ╔═╡ 9d94a64e-0dd3-4778-a6dd-79f8ff2f54fe
md"""
How does the velocity look after pushing it through the `ShiftScale`-layer?
"""

# ╔═╡ e6f6f9a6-d876-41fa-addf-01028dc74495
testVals = collect(tanh(preProcess(collect(val[t] for val in manipulatedDerVals))[1]) for t in 1:length(resultFMU1.values.t))

# ╔═╡ 18636eaa-6919-4051-801b-cb698e123b36
plot!(resultFMU1.values.t, 
      testVals; 
      label="tanh(preProcess(velocity))")

# ╔═╡ 58f83840-b948-4fa8-bd45-fff2ed61e27b
md"""
...
# ╔═╡ 8f770f53-6e26-414c-b89c-b2994af8051d
md"""
...
# ╔═╡ 6b1c65d3-413d-4e89-97fc-d3291283264c
...
# ╔═╡ 5e194df1-38e1-420b-915a-5431ce37bab3
md"""
Let's test the NeuralFMU: First, load the FMU und built a NeuralFMU from it.
"""

# ╔═╡ 5cd60aff-6bd4-4e80-bf9f-b718ec7557c5
# build NeuralFMU
neuralFMU1 = build_NFMU(fmu)

# ╔═╡ 396931df-b2c8-4f29-9782-377d7049aae2
md"""
Next, do a simulation for a given start state `x0` from *FMIZoo.jl*.
"""

# ╔═╡ d21e691a-e06a-4124-a283-7e5787c9a6b4
# simulate and plot the (uninitialized) NeuralFMU
resultNFMU1 = neuralFMU1(x0,                         # the start state to solve the ODE
                       (tStart, tStop);             # the simulation range
                       parameters=data.params,      # the parameters for the VLDM
                       showProgress=showProgress,   # show progress (or not)
                       saveat=tSave);               # the time points to save the solution at

# ╔═╡ 9aa51e84-fb55-44e3-8644-e0c37fdd313b
display(resultNFMU1)

# ╔═╡ 3c11ad0b-1bbf-40d5-910a-f63d8ff98121
md"""
...
# ╔═╡ 9fce7d6f-02f9-44e1-aac3-61f5d1717ace
begin
	# plot the NeuralFMU, original FMU and data (cumulative consumption)
	fig2 = plot(resultNFMU1; stateIndices=6:6, stateEvents=false, timeEvents=false, label="NeuralFMU (untrained)", ylabel="cumulative consumption [Ws]")
	plot!(fig2, resultFMU1; stateIndices=6:6, values=false, stateEvents=false, timeEvents=false, label="FMU")
	plot!(fig2, data.cumconsumption_t, data.cumconsumption_val, label="Data")
end

# ╔═╡ 970a1369-beea-4994-a36f-bf7ad27ef989
md"""
...
# ╔═╡ 2bd00dd0-0601-4244-a6b4-027fec22a44e
# unload FMU
fmiUnload(fmu)

# ╔═╡ 78c9042c-da02-4742-80af-b129e46573d0
# ╠═╡ disabled = true
#=╠═╡
#invalidate NeuralFMU
neuralFMU = nothing
  ╠═╡ =#

# ╔═╡ 22982af9-2505-4d1d-82ef-009f02d233f7
md"""
## 4. Training the NeuralFMU
"""

# ╔═╡ 8bc69cd1-e129-4df2-9a23-f8e2ad6d47d7
md"""
...
# ╔═╡ 91e39709-2e64-4ab7-a057-091d3da9a8a1
# prepare training data 
train_t = data.consumption_t 

# ╔═╡ 734c91a4-bec3-4807-a75c-5850c79ec499
...
# ╔═╡ db170e3c-ec7e-46e9-9450-ff9488fb1c60
md"""
...
# ╔═╡ 6e498560-ed7c-4bc7-ae2e-5edb0aa05567
...
# ╔═╡ 4a40e372-6d43-4d10-acad-23da3ef73a4c
md"""
...
# ╔═╡ d7566cd9-cc4b-4eb2-b34e-7823a6174563
...
function train!(hyper_params, ressource, ind)
    ...
    batch = batchDataSolution(neuralFMU2,                           # our NeuralFMU model
    ...
    if SCHEDULER == :Random
        ...
        scheduler = RandomScheduler(neuralFMU2, batch; applyStep=1, plotStep=0)
    elseif SCHEDULER == :Sequential
        # a scheduler that picks one batch element after another (in chronological order)
        scheduler = SequentialScheduler(neuralFMU2, batch; applyStep=1, plotStep=0)
    elseif SCHEDULER == :LossAccumulation
        ...
        scheduler = LossAccumulationScheduler(neuralFMU2, batch, lossFct; applyStep=1, plotStep=0, updateStep=1)
    else 
    ...
    loss = p -> FMIFlux.Losses.loss(neuralFMU2,                          # the NeuralFMU to simulate
    ...
    params = FMIFlux.params(neuralFMU2)
    ...
    fmiSaveParameters(neuralFMU2, joinpath(@__DIR__, "params", "$(ind).jld2"))
    
    # simulate the NeuralFMU on a validation trajectory
    resultNFMU = neuralFMU2(x0, (data_validation.consumption_t[1], data_validation.consumption_t[end]); parameters=data_validation.params, showProgress=showProgress, maxiters=1e7, saveat=data_validation.consumption_t)
...
# ╔═╡ 596ed605-f6a3-4afa-b745-78ced343420e
md"""
# 5. Results 
...
# ╔═╡ a9b112b6-e931-449f-9035-67aeb348bd54
# load our FMU (we take one from the FMIZoo.jl, exported with Dymola 2020x)
fmu2 = fmiLoad("VLDM", "Dymola", "2020x"; type=:ME)

# ╔═╡ 036fba3a-0610-4b16-8675-ecf6da6c785d
# build NeuralFMU
neuralFMU3 = build_NFMU(fmu2)

# ╔═╡ 382da1e2-45bf-4d5d-94ee-f3ac43ff8b2e
# load parameters from hyperparameter optimization
fmiLoadParameters(neuralFMU3, joinpath(@__DIR__, "plutocon_2023.jld2"))

# ╔═╡ bfa3fb02-eafd-4129-9ec6-89cf4b256f35
# simulate the NeuralFMU
resultNFMU2 =   neuralFMU3(x0,  (tStart, tStop); parameters=data.params, showProgress=showProgress, saveat=tSave) 

# ╔═╡ df77c2dd-2073-4131-b1fa-06e3e1c1048e
# and the normal FMU
resultFMU2  = fmiSimulate(fmu2, (tStart, tStop); parameters=data.params, showProgress=showProgress, saveat=tSave) 

# ╔═╡ e01e4d52-fab9-45d4-bad0-8c007fbcbd79
begin
	# plot the NeuralFMU, original FMU and data (cumulative consumption)
	fig3 = plot(resultNFMU2; stateIndices=6:6, stateEvents=false, timeEvents=false, label="NeuralFMU", ylabel="cumulative consumption [m/s]")
	plot!(fig3, resultFMU2; stateIndices=6:6, values=false, stateEvents=false, timeEvents=false, label="FMU")
	plot!(fig3, data.cumconsumption_t, data.cumconsumption_val, label="Data")
end

# ╔═╡ 54d3ebb5-81d2-420d-8f6b-edd7ec9ca768
md"""
We also have a ready-to-use function that calculates different errors and plots them.
"""

# ╔═╡ d8f29380-63d4-4053-95cb-70e9bc891852
plotCumulativeConsumption(resultNFMU2, resultFMU2, data; filename=joinpath(@__DIR__, "comparision_train_100.png"))

# ╔═╡ ee73cb3e-b3d8-45cf-8576-217f133aacdd
md"""
Because the deviation is small, let's check the last 10% of WLTC focussed, so from 90% to 100%.
"""

# ╔═╡ f06cc917-bad3-48f9-b83f-8f3a7474a816
plotCumulativeConsumption(resultNFMU2, resultFMU2, data; range=(0.9, 1.0), filename=joinpath(@__DIR__, "comparision_train_10.png"))

# ╔═╡ d96cae72-c120-48f3-a798-2d52c87ceccb
md"""
Finally, we should check the results on validation data: The full WLTC cycle.
"""

# ╔═╡ 4e53f1d8-5ec7-4a64-b85e-919f5935b875
begin
...
# ╔═╡ 41c79954-d827-4637-b607-8c35aed6c6d1
# simulate the NeuralFMU on validation data
resultNFMU3 = neuralFMU3(x0,  (tStart_validation, tStop_validation); parameters=data_validation.params, showProgress=showProgress, saveat=tSave_validation)

# ╔═╡ f7614c58-ff59-43ad-b119-5085a370bff4
# simulate the normal FMU on validation data
resultFMU3 = fmiSimulate(fmu2, (tStart_validation, tStop_validation); parameters=data_validation.params, showProgress=showProgress, saveat=tSave_validation) 

# ╔═╡ a62196f2-ee2b-40d3-9f6f-69ac0c0cab1d
plotCumulativeConsumption(resultNFMU3, resultFMU3, data_validation; filename=joinpath(@__DIR__, "comparision_validation_100.png"))

# ╔═╡ 7274c6d8-b7e0-4925-bcde-176597c3479f
md"""
... and the last 10% ...
"""

# ╔═╡ 3d6e557e-e20e-4353-a4ff-d95470f0a0aa
plotCumulativeConsumption(resultNFMU3, resultFMU3, data_validation; range=(0.9, 1.0), filename=joinpath(@__DIR__, "comparision_validation_10.png"))

# ╔═╡ 6dd97e30-8551-470d-b373-7d2d9b71b2b4
md"""
...
# ╔═╡ c3c5d544-01f6-4380-aecf-a0b1f728caa5
plotEnhancements(neuralFMU3, fmu2, data; filename=joinpath(@__DIR__, "gif_1.gif"))

# ╔═╡ 76c1e217-e008-43bb-9dae-b3902cf42702
md"""
After we finished, let's finally unload the FMU and invalidate the NeuralFMU.
"""

# ╔═╡ 7f334fec-b82e-4c7a-929e-02a6259d6375
# unload FMU / invalidate NeuralFMU
fmiUnload(fmu2)

# ╔═╡ 6288545a-650b-4047-8ca2-c2f1930e1f31
# ╠═╡ disabled = true
#=╠═╡
neuralFMU = nothing
  ╠═╡ =#

# ╔═╡ d2775e7e-5fca-4d7f-9473-c53f778092c2
md"""
...
# ╔═╡ 2a7be19c-62dd-4496-a1b1-c9fd39efdda1
md"""
...
# ╔═╡ f23ae16b-0cb0-4a04-a457-ecfc941a80e5
...
manifest_format = "2.0"
project_hash = "662c5cb31ff140294c236b4caadc65df1278824c"
